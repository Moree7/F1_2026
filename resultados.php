...
if (isset($_POST['accion']) && $_POST['accion'] === 'insertar') {
    try {
        // ON DUPLICATE KEY UPDATE: si ya existe resultado para ese piloto en esa carrera, lo actualiza
        $stmt = $conexion->prepare(
            "INSERT INTO resultados (id_carrera, id_piloto, posicion, puntos, vueltas, tiempo_total, estado_carrera, pit_stops)
             ...
             ON DUPLICATE KEY UPDATE
               ...
               vueltas        = VALUES(vueltas),
               tiempo_total   = VALUES(tiempo_total),
               estado_carrera = VALUES(estado_carrera),
               pit_stops      = VALUES(pit_stops)"
        );
        $stmt->execute([
        ...
        $msgOk = 'Resultado guardado correctamente.';
...
// Si no hay filtro seleccionado, cargamos automáticamente la carrera más reciente con resultados
if ($filtroCarrera === 0) {
    $ultCarrera = $conexion->query(
        "SELECT c.id_carrera FROM carreras c
         INNER JOIN resultados r ON r.id_carrera = c.id_carrera
         ORDER BY c.fecha DESC LIMIT 1"
    )->fetch();
    if ($ultCarrera) $filtroCarrera = (int)$ultCarrera['id_carrera'];
}

// Mostrar siempre los resultados de UNA carrera — nunca mezclar todas
if ($filtroCarrera > 0) {
    ...
    $stmtR = $conexion->query("SELECT * FROM resultados WHERE 0"); // vacío si no hay ninguna carrera
}
